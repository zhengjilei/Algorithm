 ...
 * <p>
 * 思路3：选出第 k 小的数 O(n)，次数超过一半, 则 第 n/2 + 1 小的数一定是该数了
 * 为什么是第 n/2+1 小不是 第 n/2？
 * n=奇数，例 n=11, n/2 = 5  如果选的是第5小的元素， 剩余6个大于第5小的数字可能都相等，而该数才是目标值
 * n=偶数，n=10,n/2 = 5 ，则数量超过n/2 的数一定同时是第 5 小的数 和 第 6 小的数
 * <p>
 * 如果数组中没有一个数的次数超过数组的一半呢？
 * 解决？找出第 n/2+1 小的数后，然后再遍历一遍统计次数
 * <p>
 * 缺点： 会改变原数组
 *
 * <p>
